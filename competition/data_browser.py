...
class DataBrowser(object):
    ...
    """
        ...
        d_test = AutoDLDataset(os.path.join(dataset_dir, dataset_name + '.data', "test"))
        other_info = {}
        ...
        with tf.Session() as sess:
            for _ in range(num + 1):
                try:
                    ...
                    tf.logging.info(
                        "Reached the end of dataset. " + "Return the last example."
                    )
                    break
    ...
    """
        ...
        if subset != 'train':
            if self.other_info['with_solution']:
                ...
                tf.logging.info(
                    "No solution file found for test set. " +
...
def show_examples(dataset_dir, num_examples=5, subset='train'):
    ...
    for i in range(num_examples):
        print(
            "Visualizing example {}.".format(i + 1) +
...
def main(*argv):
    ...
    tf.flags.DEFINE_string('dataset_dir', default_dataset_dir, "Path to dataset.")
    tf.flags.DEFINE_string('subset', 'train', "Can be 'train' or 'test'.")
    tf.flags.DEFINE_integer('num_examples', 5, "Number of examples to show.")
