...
def get_prediction_files(prediction_dir):
  ...
  """
    ...
    if len(task_names) > 1:
        raise ValueError(
            "Predictions of multiple tasks are found: {}!".format(prediction_files)
        )
...
def get_new_prediction_files(prediction_dir, prediction_files_so_far=None):
    ...
    new_prediction_files = [
        p for p in prediction_files if p not in prediction_files_so_far
    ]
    order_key = lambda filename: int(filename.split("_")[-1])
...
def draw_learning_curve(
    ...
    if len(timestamps) > 0:
        ...
        logger.info(
            "(2 * AUC - 1) of the latest prediction is {:.4f}.".format(latest_score)
        )
        if not latest_roc_auc == -1:
            logger.info(
                "ROC AUC of the latest prediction is {:.4f}.".format(latest_roc_auc)
            )
        if is_multiclass_task:
