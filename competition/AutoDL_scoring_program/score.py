...
import psutil
import yaml
from libscores import _HERE, get_logger, ls, mvmean, read_array, sp, tiedrank
    ...
    **kwargs
    ...
    if not le == len(scores):
        raise ValueError(
            ...
            "should be equal to the number of " + "scores {}!".format(len(scores))
        )
    for i in range(le):
...
def terminate_process(pid, create_time):
...
class LearningCurve(object):
    ...
    def get_alc(self, t0=60):
        X = [
            transform_time(t, T=self.time_budget, t0=t0)
            for t in self.timestamps if t < self.time_budget
        ]
        Y = list(self.scores.copy())[:len(X)]
        X.insert(0, 0)
        Y.insert(0, 0)
        X.append(1)
        Y.append(Y[-1])
...
class Evaluator(object):
    ...
    def fetch_ingestion_info(self):
        ...
        # Get ingestion PID
        ingestion_create_time = ingestion_info['ingestion_create_time']
        ...
        self.ingestion_create_time = ingestion_create_time
        self.time_budget = time_budget
    ...
    def kill_ingestion(self):
        terminate_process(self.ingestion_pid, self.ingestion_create_time)

    def delete_prediction_dir(self):
    ...
    def compute_error_bars(self, n=10):
        ...
        try:
            ...
            for _ in range(n):  # number of scoring
                ...
                scores.append(
                    scoring_function(np.array(new_solution), np.array(new_predictions))
                )
            mean = np.mean(scores)
            std = np.std(scores)
            var = np.var(scores)
    ...
    def score_new_predictions(self):
        new_prediction_files = self.get_new_prediction_files()
        if len(new_prediction_files) > 0:
            ...
            logger.info("[+] New prediction found. Now number of predictions " +
                                     "made = {}"\
                                     .format(len(self.prediction_files_so_far)))
            logger.info("Current area under learning curve for {}: {:.4f}"\
                                .format(self.task_name, score))
    ...
    def write_final_score_to_file(self):
        score = self.learning_curve.get_alc()
        with open(os.path.join(self.score_dir, 'final_score.txt'), "w") as file:
