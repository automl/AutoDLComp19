...
import psutil
import yaml
from libscores import ls, mvmean, read_array, sp, tiedrank
...
def draw_learning_curve(
    solution_dir, prediction_files, scoring_function, output_dir, basename, start,
    is_multiclass_task
):
...
def update_score_and_learning_curve(
    prediction_dir, basename, start, solution_dir, scoring_function, score_dir,
    is_multiclass_task
):
    prediction_files = get_prediction_files(prediction_dir)
    alc = 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        while (time.time() < ingestion_start + time_budget):
            ...
            if is_process_alive(ingestion_pid):
                ...
                logger.info(
                    "Detected time budget is used up. Killing ingestion and " +
                    "terminating scoring..."
                )
    ...
    score = update_score_and_learning_curve(
        prediction_dir, basename, ingestion_start, solution_dir, scoring_function,
