...
def generate_all_commands(commands_file, remove_datasets, optimize_generalist):
    commands = [
        construct_command(dataset, worker, optimize_generalist)
        for dataset in train_datasets
        for worker in [True, False]
        if not dataset in remove_datasets
    ]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--command_file_name", default="hpo_args_individualists_v1.args", help=" ")
