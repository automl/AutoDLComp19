...
def _run_on_dataset(dataset, config_experiment_path, model_config, dataset_dir, n_repeat,
    ...
    for _ in range(n_repeat):
        score = evaluate_on_dataset(dataset_dir=str(dataset_path), code_dir="src",
                                    ...
                                    time_budget_approx=time_budget_approx, overwrite=True,
                                    model_config_name=None, model_config=model_config)
...
class SingleWorker(Worker):
    ...
    def compute(self, config_id, config, budget, *args, **kwargs):
        ...
        repetition_scores, repetition_mean = _run_on_dataset(self.dataset, config_experiment_path,
                                                             model_config,
...
class AggregateWorker(Worker):
    ...
    def compute(self, config_id, config, budget, *args, **kwargs):
        ...
        score_results_tuples = [
            _run_on_dataset(dataset, config_experiment_path / dataset, model_config,
                            dataset_dir=self._dataset_dir, n_repeat=self.n_repeat,
                            time_budget=budget, time_budget_approx=self.time_budget_approx) for
...
if __name__ == "__main__":
    ...
    worker = AggregateWorker(working_directory='experiments/test/test_aggregate_worker', n_repeat=1,
                             run_id='0', time_budget=60, time_budget_approx=1200)
