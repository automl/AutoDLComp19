...
def get_model(model_name, model_dir, dropout, num_classes):
    ...
    if 'squeezenet' in model_name:
        ...
        model = DenseNet(growth_rate=8, block_config=(2, 4, 8, 4),
                         num_init_features=32, bn_size=2, drop_rate=dropout,
                         num_classes=num_classes).cuda()
        if '32' in model_name:
