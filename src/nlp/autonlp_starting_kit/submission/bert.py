...
class NLPBertClassifier(nn.Module):
    def __init__(self, metadata, bert_metadata, pretrained=None, vocab_size=None, encoder_layers=1, attn_heads=2,
                 classifier_layers=3, classifier_units=768, finetuning=True):
        """
        metadata: dict
        ...
        self.layers = encoder_layers if pretrained is None else self.bert_metadata[self.language]['layers']
        self.heads = attn_heads if pretrained is None else self.bert_metadata[self.language]['heads']
        self.vocab_size = vocab_size if pretrained is None else self.bert_metadata[self.language]['vocab']
