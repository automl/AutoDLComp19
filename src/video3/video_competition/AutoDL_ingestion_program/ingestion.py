...
if __name__=="__main__":
    ...
    pytorch_basename = basename + ".pytorch"
    pytorch_train = os.path.join(dataset_dir, pytorch_basename, "train")
    pytorch_test = os.path.join(dataset_dir, pytorch_basename, "test")
    ...
    try:
      ...
      # Check if submission needs pytorch data
      if hasattr(M, "use_dataset_path") and M.use_dataset_path:
        get_train_dataset = lambda: pytorch_train
        get_test_dataset = lambda: pytorch_test
      else:
        get_train_dataset = lambda: D_train.get_dataset()
        get_test_dataset = lambda: D_test.get_dataset()
      ...
      while(not (use_done_training_api and M.done_training)):
        ...
        logger.info("Begin testing the model by making predictions " +
                     "on test set...")
        Y_pred = M.test(get_test_dataset(),
                        remaining_time_budget=remaining_time_budget)
