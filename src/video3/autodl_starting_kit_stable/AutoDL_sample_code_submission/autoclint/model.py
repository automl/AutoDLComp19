...
import autoclint.skeleton as skeleton
import numpy as np
import tensorflow as tf
import torch
import torchvision as tv
from autoclint.architectures.resnet import ResNet18
from autoclint.skeleton.projects import LogicModel, get_logger
from autoclint.skeleton.projects.others import AUC, NBAC
...
class Model(LogicModel):
    def __init__(self, metadata, parser_args):
        super(Model, self).__init__(metadata)
        self.use_test_time_augmentation = False
        self.lr = parser_args.lr
        self.optimizer_type = parser_args.optimizer
    ...
    def init_opt(self):
        ...
        scheduler_lr = skeleton.optim.gradual_warm_up(
            skeleton.optim.get_reduce_on_plateau_scheduler(
                self.lr * lr_multiplier / warmup_multiplier,  # change to 0.025
                patience=10,
                factor=.5,
        ...
        self.optimizer = skeleton.optim.ScheduledOptimizer(
            params,
            eval("torch.optim." + self.optimizer_type),
            # skeleton.optim.SGDW,
