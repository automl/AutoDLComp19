...
import ops.average_precision_calculator as ap_calculator
import ops.mean_average_precision_calculator as map_calculator
import numpy
import torch
...
def calculate_gap(predictions, actuals, top_k=20):
  ...
  """
    ...
    gap_calculator.accumulate(
        ...
        torch.sum(torch.stack(num_positives)).item()
    )
...
class EvaluationMetrics(object):
    ...
    """
        ...
        self.global_ap_calculator.accumulate(
            flatten(sparse_predictions), flatten(sparse_labels),
            torch.sum(torch.stack(num_positives))
        )
