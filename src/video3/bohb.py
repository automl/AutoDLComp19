...
class ChallengeWorker(Worker):
    ...
    def compute(self, config, budget, working_directory, *args, **kwargs):  # noqa: C901
        ...
        model, optimizer = Load_model_and_optimizer(parser_args, config)
        # define loss function (criterion) and optimizer
        if (
            parser_args.loss_type == 'nll' and
        ...
        if parser_args.training:
            with open(
                os.path.join(parser_args.working_directory, 'parser_args.txt'), 'w'
            ) as f:
                f.write(str(parser_args))
        ############################################################
        # Training
        if parser_args.temporal_pool and not parser_args.resume:
            make_temporal_pool(model.module.base_model, parser_args.num_segments)
        ...
        for epoch in range(parser_args.start_epoch, epoches_to_train):
            ...
            adjust_learning_rate(optimizer, epoch, parser_args.lr_steps, exp_num)
            # train for one epoch
            budget_counter -= 1
            train_budget = 1 if budget_counter > 1 else budget_counter
            _ = train(self.train_loader, model, criterion, optimizer, epoch, train_budget)
            ############################################################
            ...
            # overfitting
            is_best = prec1 > parser_args.best_prec1
            if is_best:
                saturate_cnt = 0
            else:
            ...
            if parser_args.best_prec1 < prec1:
                parser_args.best_prec1 = prec1
                save_checkpoint(
                    {
                        'epoch': epoch + 1,
                        'arch': parser_args.arch,
                        'state_dict': model.state_dict(),
                        'best_prec1': parser_args.best_prec1,
