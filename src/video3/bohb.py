...
class ChallengeWorker(Worker):
    ...
    def compute(self, config, budget, working_directory, *args, **kwargs):  # noqa: C901
        ...
        if (parser_args.loss_type == 'nll'
            and parser_args.classification_type == 'multiclass'):
            criterion = torch.nn.CrossEntropyLoss().cuda()
            if parser_args.print:
                print("Using CrossEntropyLoss")
        elif (parser_args.loss_type == 'nll'
            and parser_args.classification_type == 'multilabel'):
        ...
        if parser_args.training:
            with open(os.path.join(parser_args.working_directory,
...
def train(train_loader, model, criterion, optimizer, epoch, budget):
    ...
    if parser_args.classification_type == 'multiclass':
        top5 = AverageMeter()
        true_pos, false_pos, false_neg, true_neg, precision, recall = (
                                                                    None,)*6
...
def validate(val_loader, model, criterion, logger=None):
    ...
    if parser_args.print:
        if parser_args.classification_type == 'multiclass':
            ...
            print(('Testing Results: F2@1 {:.3f} Prec@1 {:.3f}'
                   ' Rec@1 {:.3f} Loss {:.5f}'
                        .format(top1.avg, 
                                precision.avg, 
                                recall.avg,
