...
def calculate_precision_at_equal_recall_rate(predictions, actuals):
  ...
  for row in numpy.arange(num_videos):
    num_labels = int(torch.sum(actuals[row]))
...
def calculate_gap(predictions, actuals, top_k=20):
  ...
  gap_calculator.accumulate(flatten(sparse_predictions), flatten(sparse_labels), torch.sum(torch.stack(num_positives)).item())
...
def top_k_by_class(predictions, labels, k=20):
  ...
  out_true_positives = [torch.sum(labels[:,i]) for i in range(num_classes)]
...
class EvaluationMetrics(object):
  ...
  def accumulate(self, predictions, labels, loss):
    ...
    self.global_ap_calculator.accumulate(flatten(sparse_predictions), flatten(sparse_labels), torch.sum(torch.stack(num_positives)))
