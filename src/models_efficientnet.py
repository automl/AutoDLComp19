...
class SqueezeExcite(nn.Module):
    def __init__(self, inplanes, se_ratio, prob=0.0):
...
class MBConv(nn.Module):
    def __init__(
        ...
        layer.append(InvertedResidual(inplanes, planes, kernel_size, stride, expand, se_ratio))
        for l in range(1, repeat):
            # https://arxiv.org/pdf/1603.09382.pdf
            prob = 1.0 - (count_layer + l) / sum_layer * (1 - pl)
            layer.append(
                InvertedResidual(planes, planes, kernel_size, 1, expand, se_ratio, prob=prob)
            )
            # if l == repeat - 1:
...
class EfficientNet(nn.Module):
    ...
    def __init__(
        ...
        self.stage1 = nn.Sequential(
            ...
            nn.BatchNorm2d(
                channels[0], momentum=0.01, eps=1e-05, affine=True, track_running_stats=True
            ),
        )
        se_ratio = 4
