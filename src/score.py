...
def get_prediction_files(prediction_dir):
  ...
  """
    ...
    task_names = set([os.path.basename(f.split(".")[-2]) for f in prediction_files])
    if len(task_names) > 1:
        raise ValueError("Predictions of multiple tasks are found: {}!".format(prediction_files))
...
def get_new_prediction_files(prediction_dir, prediction_files_so_far=None):
    ...
    new_prediction_files = [p for p in prediction_files if p not in prediction_files_so_far]
    order_key = lambda filename: int(filename.split("_")[-1])
...
def draw_learning_curve(
    ...
    if len(timestamps) > 0:
        ...
        logger.info("(2 * AUC - 1) of the latest prediction is {:.4f}.".format(latest_score))
        if not latest_roc_auc == -1:
            logger.info("ROC AUC of the latest prediction is {:.4f}.".format(latest_roc_auc))
        if is_multiclass_task:
            sorted_pairs_acc = sorted(zip(timestamps, accuracy_scores))
            latest_acc = sorted_pairs_acc[-1][1]
            logger.info("Accuracy of the latest prediction is {:.4f}.".format(latest_acc))
    X = [t for t, _ in sorted_pairs]
    Y = [s for _, s in sorted_pairs]
    alc, ax = plot_learning_curve(
        X, Y, start_time=start, time_budget=time_budget, task_name=basename
    )
...
def update_score_and_learning_curve(
    prediction_dir, basename, start, solution_dir, scoring_function, score_dir, is_multiclass_task
):
    prediction_files = get_prediction_files(prediction_dir)
    alc = 0
    alc, time_used = draw_learning_curve(
...
if __name__ == "__main__":
    ...
    for i in range(wait_time):
        ...
        if not ingestion_info is None:
            logger.info(
                "Detected the start of ingestion after {} ".format(i) + "seconds. Start scoring."
            )
            break
        time.sleep(1)
    else:
    ...
    if "time_budget" in ingestion_info:
        if not time_budget == ingestion_info["time_budget"]:
            logger.warning(
                "Different time budgets passed to ingestion and "
    ...
    try:
        ...
        while time.time() < ingestion_start + time_budget:
            ...
            if num_preds_new > num_preds:
                ...
                new_scores = [
                    scoring_function(solution, read_array(pred)) for pred in new_prediction_files
                ]
                ...
                logger.info(
                    "Current area under learning curve for {}: {:.4f}".format(basename, score)
                )
    ...
    logger.info("Final area under learning curve for {}: {:.4f}".format(basename, score))
    ...
    if not scoring_success:
        ...
        if end_info_dict["ingestion_success"] == 0:
            ...
            logger.info(
                ...
                + "The score of your algorithm on the task '{}' is: {:.6f}.".format(basename, score)
            )
