...
class Model(algorithm.Algorithm):
    def __init__(self, metadata):
        ...
        # DEAFULT LARGER SIZED IMAGES TO 224x224
        if row_count >= 32 or col_count >= 32:
        ...
        inm = ImageNetModels()
        model_ft, _ = inm.initialize_model(
            model_name, self.output_dim, feature_extract, use_pretrained=True
        )

        # Print the model we just instantiated
        print(model_ft)
        print(model_ft.fc.weight.data)
        print(model_ft.fc.bias.data)

        # PRINT UPDATABLE PARAMS
        ...
        self.num_epochs_we_want_to_train = 20

        # no of examples at each step/batch
        self.batch_size = 64

        print_log(
            "MAX POSSIBLE BATCHES ==== >>> ",
            np.ceil(self.num_examples_train / self.batch_size),
        )
    ...
    """
        ...
        # print_log("NEW RC: ", new_row_count, " ... NEW CC: ", new_col_count)
    ...
    """
        ...
        if train:
            ...
            print_log(
                " GET DATALOADER CALLED FOR TESTING! ...................................."
            )
        return loader
    ...
    """
        ...
        for i in range(steps):
            ...
            # print(i, "TRAINLOOP SHAPES: ", images.shape, labels.shape)
            # print(i, "TRAINLOOP SHAPES: ", images[0].shape, images[1].shape, images[2].shape, images[3].shape)
            # print(i, "TRAINLOOP SHAPES: ", images[0][0].shape, images[0][1].shape, images[0][2].shape)
    ...
    def train(self, dataset, remaining_time_budget=None):
        print_log(
            "METHOD TRAIN -------->>> REMAINING TIME BUDGET ::>> ", remaining_time_budget
        )
        steps_to_train = self.get_steps_to_train(remaining_time_budget)
        print_log(" BATCHES TO TRAIN: ---> ", steps_to_train)
        if steps_to_train <= 0:
            print_log(
    ...
    """
        if not remaining_time_budget:  # This is never true in the competition anyway
            remaining_time_budget = 3600  # if no time limit is given, set to 20min
