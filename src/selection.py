...
def baseline_selector(autodl_model, dataset, modelargs):
...
def test_selector(autodl_model, dataset, selection_args):
    ...
    scheduler = None
    selection_args.update(OrderedDict({
        'num_classes': dataset.num_classes,
        'classification_type': 'multilabel' if dataset.is_multilabel else 'multiclass',
    }))
    if dataset.mean_shape[0] == 1:  # image network
        kakaomodel = torch.hub.load('kakaobrain/autoclint', 'KakaoModel', autodl_model.metadata)
        ...
        autodl_model.train = EarlyStop(
            kakaomodel.train,
            autodl_model
        )
        autodl_model.test = EarlyStop(
            kakaomodel.test,
            autodl_model
        )
