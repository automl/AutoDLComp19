    ...
    def __init__(self, dataset="ucf101", data_dir="."):
        if dataset == "ucf101":
            ...
            self.train_loader = data.DataLoader(
                dataset=UCF101(data_dir=data_dir, type="train"),
                batch_size=1,
                shuffle=True,
            )
            self.valid_loader = data.DataLoader(
                dataset=UCF101(data_dir=data_dir, type="valid"),
                batch_size=1,
                shuffle=True,
            )
            self.test_loader = data.DataLoader(
                dataset=UCF101(data_dir=data_dir, type="test"),
                batch_size=1,
                shuffle=False,
            )
...
class UCF101(data.Dataset):
    ...
    def __init__(self, data_dir, type):
        ...
        index_path = os.path.join(
            os.path.abspath(data_dir), "aaa_recognition/classInd.txt"
        )
        train_list_path = os.path.join(
            os.path.abspath(data_dir), "aaa_recognition/trainlist01.txt"
        )
        test_list_path = os.path.join(
            os.path.abspath(data_dir), "aaa_recognition/testlist01.txt"
        )
