...
class ModelSelect(nn.Module):
    def __init__(self, nb_frames, output_size, model_type):
        ...
        if model_type == 'i3d':
            ...
            self.model = TimeceptionWrapper(nb_frames = nb_frames, base_model = 'resnet50', output_size = output_size)
        elif model_type == 'timeception152':
            self.model = TimeceptionWrapper(nb_frames = nb_frames, base_model = 'resnet152', output_size = output_size)
    ...
    def forward(self, x):
        ...
        if self.model_type == 'i3d' or self.model_type == 'slowfast50' or self.model_type == 'slowfast152':
