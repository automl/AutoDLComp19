...
def process_video(cfg, model, video):
    ...
    segment_array = prep.get_segment_array(
        video=video,
...
def train(cfg):
    dataset = Dataset(dataset=cfg["dataset_name"], data_dir=cfg["dataset_data_dir"])

    train_loader, valid_loader, test_loader = dataset.get_data_loader()

    model = models.ModelSelect(
        nb_frames=cfg["video_segment_length"] * cfg["video_segment_count"],
