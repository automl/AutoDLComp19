...
class SlowFast(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(
            self.fast_inplanes + self.slow_inplanes, class_num, bias=False
        )
    ...
    def _make_layer_fast(
        self, block, planes, blocks, shortcut_type, stride=1, head_conv=1
    ):
        downsample = None
...
if __name__ == "__main__":
    num_classes = 174
    input_tensor = torch.autograd.Variable(torch.rand(2, 3, 64, 224, 224))
    model = SlowFast50(class_num=num_classes)
