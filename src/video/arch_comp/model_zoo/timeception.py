...
class ChannelShuffleLayer(Module):
    ...
    def forward(self, input):
        ...
        tensor = input.view(
            n_samples, n_groups, n_channels_per_group, n_timesteps, side_dim1, side_dim2
        )
...
class Timeception(Module):
    ...
    def __init__(self, input_shape, n_layers=4, n_groups=8, is_dilated=True):
        ...
        n_channels_out = self.__define_timeception_layers(
            input_shape, n_layers, n_groups, expansion_factor, is_dilated
        )
    ...
    def forward(self, input):
        ...
        output = self.__call_timeception_layers(
            input, n_layers, n_groups, expansion_factor
        )

        return output

    def __define_timeception_layers(
        self, input_shape, n_layers, n_groups, expansion_factor, is_dilated
    ):
        """
        ...
        for i in range(n_layers):
            ...
            self.__define_grouped_convolutions(
                input_shape, n_groups, n_channels_per_branch, is_dilated, layer_num
            )
    ...
    def __define_grouped_convolutions(
        self, input_shape, n_groups, n_channels_per_branch, is_dilated, layer_num
    ):
        """
        ...
        for idx_group in range(n_groups):
            ...
            self.__define_temporal_convolutional_block(
    ...
    def __define_temporal_convolutional_block(
        ...
        layer = DepthwiseConv1DLayer(
            dw_input_shape, kernel_sizes[0], dilation_rates[0], layer_name
        )
        setattr(self, layer_name, layer)
        ...
        layer = DepthwiseConv1DLayer(
            dw_input_shape, kernel_sizes[1], dilation_rates[1], layer_name
        )
        setattr(self, layer_name, layer)
        ...
        layer = DepthwiseConv1DLayer(
            dw_input_shape, kernel_sizes[2], dilation_rates[2], layer_name
        )
        setattr(self, layer_name, layer)
    ...
    def __call_grouped_convolutions(self, tensor_input, n_groups, layer_num):
        ...
        for idx_group in range(n_groups):
            ...
            tensor = self.__call_temporal_convolutional_block(
                tensor, layer_num, group_num
            )
    ...
    def __get_n_channels_per_branch(self, n_groups, expansion_factor, n_channels_in):
        n_branches = 5
        n_channels_per_branch = int(
            n_channels_in * expansion_factor / float(n_branches * n_groups)
        )
...
class TimeceptionWrapper(torch.nn.Module):
    def __init__(self, nb_frames, output_size, base_model="resnet50"):
        ...
        self.fc = torch.nn.Linear(
            in_features=int(nb_frames * 128 * 7 * 7 * 5 / 4), out_features=output_size
        )
