...
from keras.layers import (Dense, Flatten, Dropout, ZeroPadding3D, Input, 
            Activation, BatchNormalization, add, Reshape, GlobalAveragePooling2D)
...
curr_time = [currentDT.day, currentDT.month, currentDT.hour, currentDT.minute, currentDT.second]
result_dir = os.path.join(working_dir, 'res/bohb_results_for_{}_niter:{}_Date:{}_{}_{}:{}:{}/'.format(dataset, n_bohb_iterations, *curr_time))
try:
...
initial_configs_file = os.path.join(working_dir, '/random_init/bohb_{}_result.pkl'.format(dataset))
    ...
    def train(self, dataset, remaining_time_budget=None):
        ...
        if True:
            ...
            if continue_run == True:
                ...
                result_logger = hpres.json_result_logger(directory=continue_path, overwrite=False)
            else:
...
# Models ################################################################
class VariousModels():
    def __init__(self, output_dim, model_name, seq_length, image_shape, config,
                 #saved_model=None, 
                 features_length=2048, lr=1e-5, dc=1e-6):
        """
        `model_name` = one of:
            c3d_reduced
        ...
        if model_name == 'c3d_reduced':
            ...
            self.model = self.c3d_reduced(neurons=config['neurons'],
    ...
    def c3d_reduced(self, neurons=2042, dropout=0.5):
        ...
        model.add(Conv3D(32, (3, 3, 3), activation="relu", padding="same",
                         name="conv1", input_shape=self.input_shape, strides=(1, 1, 1)))
        model.add(MaxPooling3D(pool_size=(4, 4, 4), strides=(2, 2, 2),
                               padding='valid', name='pool1'))
        # 2nd layer group
        model.add(Conv3D(32, (3, 3, 3), activation="relu", padding="same",
                         name="conv2", strides=(1, 1, 1)))
        model.add(MaxPooling3D(pool_size=(4, 4, 4), strides=(2, 2, 2),
...
class AUTOMLWorker(Worker):
    ...
    def setup(self, *args, **kwargs):
        ...
        if kwargs['train'] :
        # dataset = dataset.map(lambda *x: (self.preprocess_tensor_4d(x[0]), x[1]))
            self.train_input_fn = lambda: self.input_function(self.dataset, is_training=True, train=True)
            self.val_input_fn = lambda: self.input_function(self.dataset, is_training=True, train=False)
        else:
            self.test_input_fn = lambda: self.input_function(self.dataset, is_training=False)
    ...
    def train(self, config, perc_steps_per_epoch=1.0):
        ...
        steps_train = int(perc_steps_per_epoch * (self.steps_per_epoch * self.test_val_split))
        logger.info('\n Train for {} steps\n'.format(steps_train))
        classifier.train(input_fn=self.train_input_fn,
                                steps=steps_train)
        # evaluate classifier on full eval dataset
        steps_val = int((self.steps_per_epoch * (1 - self.test_val_split)))
        logger.info('\n Evaluate for {} steps\n'.format(steps_val))
        result = classifier.evaluate(input_fn=self.val_input_fn,
                                          steps=steps_val,)
    ...
    def compute(self, config, budget, *args, **kwargs):
        ...
        """
        # Training params
        result = self.train(config=config,
                            perc_steps_per_epoch=budget)
        return({
    ...
    def train_single_network(self, config, perc_steps_per_epoch=1.0):
        ...
        self.classifier.train(input_fn=self.train_input_fn,
                                steps=steps)
    ...
    def get_configspace(self):
        ...
        ################################################################
        #TRAINING
        lr = CSH.UniformFloatHyperparameter('lr', 1e-7, 0.5, default_value=0.001, log=True)
        weight_decay = CSH.UniformFloatHyperparameter('weight_decay', 5e-7, 0.05, default_value=0.0005)
        ### FC Layers ###
    ...
    def model_fn(self, features, labels, mode, params):
        ...
        while True:
            ...
            hidden_layer = tf.layers.max_pooling3d(inputs=hidden_layer,
                            pool_size=pool_size,
                            strides=[min(2, pool_size[0]),
                                 min(3, pool_size[1]), min(3, pool_size[2])],
                            padding='valid',
                            data_format='channels_last')
            #logger.info("hidden layer after pooling: {}".format(hidden_layer))
            if filter_counter == 2: # get_num_entries(hidden_layer) < REASONABLE_NUM_ENTRIES or 
                break
        ...
        hidden_layer = tf.layers.dropout(
                inputs=hidden_layer, rate=params['dropout'],
    ...
    def input_function(self, dataset, is_training, train=None):
        ...
        if is_training:
            ...
            if train:
                ...
                # Shuffle input examples
                train_dataset = train_dataset.shuffle(buffer_size=self.default_shuffle_buffer)
                ...
                val_dataset = val_dataset.batch(batch_size=self.batch_size, drop_remainder=True)
