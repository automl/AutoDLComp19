...
class Validator(object):
    ...
    def validate_runtime(
        ...
        feature_times = False
        if (
            test_scenario.feature_cost_data is not None and
            test_scenario.performance_type[0] == "runtime"
        ):
        ...
        for inst, schedule in schedules.items():
            ...
            for algo, budget in schedule:
                ...
                if (
                    time <= budget and used_time <= test_scenario.algorithm_cutoff_time and
    ...
    def validate_quality(
        ...
        for inst, schedule in schedules.items():
            if len(schedule) > 1:
                self.logger.error("AutoFolio_old does not support schedules for solution quality")
                sys.exit(9)
