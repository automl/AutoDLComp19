...
class AutoFolio(object):
    def __init__(self, random_seed: int = 12345):
        ...
        logging.info("Init AutoFolio_old")
        self._root_logger = logging.getLogger()
        self.logger = logging.getLogger("AutoFolio_old")
        self.cs = None
    ...
    def run_cli(self, args=None):
        """
            main method of AutoFolio_old based on command line interface
        """
    ...
    def _outer_cv(
        ...
        for cv_fold in outer_cv_folds:
            ...
            # Use ‘AutoFolio_old.get_tuned_config()’ to tune on inner-cv
            config = self.get_tuned_config(
                outer_training, autofolio_config=autofolio_config, seed=smac_seed
            )

            # Use `AutoFolio_old.run_fold()’ to get the performance on the outer split
    ...
    def get_cs(self, scenario: ASlibScenario, autofolio_config: dict = None):
        """
            returns the parameter configuration space of AutoFolio_old
    ...
    def fit(self, scenario: ASlibScenario, config: Configuration):
        """
            fit AutoFolio_old on given ASlib Scenario
